...
阶段划分与过关标准以 [learning-path.zh.md](learning-path.zh.md) 为准，本表只登记执行状态与本区产出。状态取值：未开始 / 进行中 / 完成 / 暂缓；完成一行时同步登记产出链接。"暂缓"表示有意延后，回填顺序见各行备注。"阶段 0"不属于 learning-path 的七阶段，是本区附加的入门实验（learning-path"读者与前置"一节指向它）。
